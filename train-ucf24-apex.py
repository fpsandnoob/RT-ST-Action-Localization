...
parser.add_argument('--net', default='RFB_E_vgg', help='net type')
parser.add_argument('--basenet', default='/home/user/git/PyTorch-MFNet/exps/models/PyTorch-MFNet_ep-0054.pth', help='pretrained base model')
...
parser.add_argument('--batch_size', default=36, type=int, help='Batch size for training')
...
parser.add_argument('--ngpu', default=4, type=str2bool, help='Use cuda to train model')
...
parser.add_argument('--data_root', default='/mnt/data/Action/data/ucf24/', help='Location of VOC root directory')
parser.add_argument('--save_root', default='/mnt/data/Action/data/ucf24/', help='Location to save checkpoint models')
...
if args.net == 'RFB_vgg':
    from RFB_Net_E_vgg_nofusion import build_net as build_ssd
elif args.net == 'RFB_E_vgg':
    from RFB_Net_E_vgg import build_net as build_ssd
else:
...
def main():
    ...
    args.exp_name = 'CONV-kinetics-apex1-SSD-{}-{}-bs-{}-lr-{:05d}'.format(args.dataset,
                                                                       args.input_type, args.batch_size, int(args.lr*100000))
    ...
    net = build_ssd("train", 300, args.num_classes)
    ...
    def weights_init(m):
        if isinstance(m, nn.Conv2d):
            ...
            if m.bias is not None and isinstance(m.bias.data, torch.cuda.FloatTensor):
                m.bias.data.zero_()
    ...
    if args.input_type == 'fastOF':
        ...
        # vgg_weights = torch.load('/home/user/git/PyTorch-MFNet/exps/models/PyTorch-MFNet_ep-0054.pth')
        # vgg_weights = torch.load("weights/vgg16_reducedfc.pth")
        print('Loading base network...')
        vgg_weights = torch.load("/mnt/data/Action/data/ucf24/ucf24/cache/"
                                 "CONV-SW-4-SSD-ucf24-rgb-bs-40-vgg16-lr-00100/ssd300_ucf24_57500.pth")
        # net.base.load_state_dict(vgg_weights)
        from collections import OrderedDict
        new_state_dict = OrderedDict()
        for k, v in vgg_weights.items():
            namekey = k[7:]
            # namekey = namekey.replace("features.", "")
            if namekey[:5] == 'point':
                continue
            new_state_dict[namekey] = v
        net.load_state_dict(new_state_dict, strict=False)
        # net.base.load_state_dict(vgg_weights, strict=True)

    # path = './pwc_net.pth.tar'
    path = None
    if path is not None:
        data = torch.load(path)
        if 'state_dict' in data.keys():
            net.flow.load_state_dict(data['state_dict'])
        else:
            net.flow.load_state_dict(data)
    ...
    for name, param in parameter_dict.items():
        # if name.find("flow") > -1:
        #     print(name, 'layer parameters will be trained @ {}'.format(args.lr * 0.1))
    ...
    # net = apex.fp16_utils.network_to_half(net)
    ...
    for val_itr in range(len(val_data_loader)):
        ...
        for b in range(batch_size):
            ...
            for cl_ind in range(1, num_classes):
                ...
                if scores.shape == torch.Size([0]):
                    # print(len(''), ' dim ==0 ')
                    det_boxes[cl_ind - 1].append(np.asarray([]))
                    continue
                if scores.shape == torch.Size([]):
                    scores = torch.cuda.FloatTensor([scores])
