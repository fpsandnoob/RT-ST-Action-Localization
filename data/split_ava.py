...
parser.add_argument("--video_dir", default="/mnt/data/Action/data/video", help="Videos path.")
parser.add_argument("--annot_file", default="/home/user/git/realtime-action-detection/data/ava/ava_train_v2.1.csv",
                    help="Anotation file path.")
parser.add_argument("--actionlist_file",
                    default="/home/user/git/realtime-action-detection/data/ava/ava_action_list_v2.1.pbtxt",
                    help="Action list file path.")
parser.add_argument("--output_dir", default="/mnt/data/Action/data/ava_frames", help="Output path.")
...
for annot_number in tqdm.tqdm(annot_range):
    ...
    ffmpeg_command = 'rm %(outpath)s; \
                      ...
                      -frames:v 1 %(outpath)s 1>/dev/null 2>&1' % {
    ...
    ffmpeg_command = 'rm %(outpath)s;  \
                      ...
                      -t %(clip_length)d %(outpath)s 1>/dev/null 2>&1' % {
    ...
    # outdir_keyframe_anno = os.path.join(outdir, 'anno', video_id)
    # mkdir_p(outdir_keyframe_anno)
    # outpath_visu_anno = os.path.join(
    #     outdir_keyframe_anno,
    #     '%d_%s_%d.txt' % (int(middle_frame_timestamp), action_id, annot_number))
    # with open(outpath_visu_anno, "a") as f:
    #     f.write()
