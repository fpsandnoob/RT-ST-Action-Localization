...
class UCF24Detection(data.Dataset):
    ...
    def pull_item(self, index):
        ...
        if os.path.exists(self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num - 1)):
            img_name_0 = self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num - 1)
        else:
            img_name_0 = self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num)
        if os.path.exists(self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num + 1)):
            img_name_2 = self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num + 1)
        else:
            img_name_2 = self._imgpath + '/{:s}/{:05d}.jpg'.format(videoname, frame_num)
            # print(img_name)
        img = cv2.imread(img_name)
        img_1 = cv2.imread(img_name.replace("rgb-images", "brox-images"))
        height, width, channels = img.shape
        img_0 = cv2.imread(img_name_0.replace("rgb-images", "brox-images"))
        img_2 = cv2.imread(img_name_2.replace("rgb-images", "brox-images"))
        ...
        if self.transform is not None:
            ...
            if self.image_set == "train":
                img, boxes, labels = self.transform(np.array([img_0, img_1, img_2, img]), target[:, :4], target[:, 4])
            else:
                img, boxes, labels = self.transform(img, target[:, :4], target[:, 4])
                img_0, _, __ = self.transform(img_0, target[:, :4], target[:, 4])
                img_2, _, __ = self.transform(img_2, target[:, :4], target[:, 4])
                img_1, _, __ = self.transform(img_1, target[:, :4], target[:, 4])
                img = np.array([img_0, img_1, img_2, img])
            img_0 = img[0] + (104, 117, 123)
            img_2 = img[2] + (104, 117, 123)
            _img = img[1] + (104, 117, 123)
            # for i in boxes:
            #     xmin = int(i[0] * 300)
            #     ymin = int(i[1]*300)
            #     xmax = int(i[2]*300)
            #     ymax = int(i[3]*300)
            #     cv2.rectangle(img[1], (xmin, ymin), (xmax, ymax), (0, 255, 0), 3)
            #     # img = img.transpose(2, 0, 1)
            # cv2.imwrite("0.png", img[1])

            img = img[3][:, :, (2, 1, 0)]
            _img = _img[:, :, (2, 1, 0)]
            img_0 = img_0[:, :, (2, 1, 0)]
            img_2 = img_2[:, :, (2, 1, 0)]
            # img = img.transpose(2, 0, 1)l
            # cv2.imwrite("0.jpg", img[1])
            # cv2.imwrite("1.jpg", _img)
            # cv2.imwrite("2.jpg", img_0)
            # cv2.imwrite("3.jpg", img_2)

            target = np.hstack((boxes, np.expand_dims(labels, axis=1)))
        # print(height, width,target)
        x = torch.from_numpy(np.array([img_0, _img, img_2, img/255.], dtype=np.float32)).permute(0, 3, 1, 2)
