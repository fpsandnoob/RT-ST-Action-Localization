...
class RFBNet(nn.Module):
    ...
    def __init__(self, phase, size, base, base_flo, extras, head, num_classes, flow_path="./pwc_net.pth.tar",
        ...
        self.loc_ = nn.ModuleList(head[2])
        self.conf_ = nn.ModuleList(head[3])
        ...
        if fusion:
            ...
            self.pointwise_conv = BasicConv(6, 3, kernel_size=1)
        elif sw:
            self.pointwise_conv_ = BasicConv(2, 3, kernel_size=1)
    ...
    def forward(self, data_x):
        ...
        if self.fusion or self.sw:
            ...
            f_2 = torch.cat([F.upsample_bilinear(x_0, size=(320, 320)), F.upsample_bilinear(x_2, size=(320, 320))], 1)
            ...
            f_2 = self.flow_correct(f_2)
            flo = f_2 * 20.0
            # flo = torch.cat([f_0, f_1, f_2], 1) * 20.0
        ...
        if self.fusion or self.sw:
            for (x, l, c, l_, c_) in zip(sources, self.loc, self.conf, self.loc_, self.conf_):
                loc.append(l(l_(x)).permute(0, 2, 3, 1).contiguous())
                conf.append(c(c_(x)).permute(0, 2, 3, 1).contiguous())
        else:
...
def multibox(size, vgg, vgg_flow, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        if k == 0:
            loc_layers_ += [nn.Conv2d(1024,
                                     512, kernel_size=1, padding=0)]
            loc_layers += [nn.Conv2d(512,
                                 cfg[k] * 4, kernel_size=3, padding=1)]
            conf_layers_ += [nn.Conv2d(1024,
                                     512, kernel_size=1, padding=0)]
            conf_layers +=[nn.Conv2d(512,
                                 cfg[k] * num_classes, kernel_size=3, padding=1)]
        else:
            loc_layers_ += [nn.Conv2d(vgg[v].out_channels * 2,
                                     vgg[v].out_channels, kernel_size=1, padding=0)]
            loc_layers += [nn.Conv2d(vgg[v].out_channels,
                                     cfg[k] * 4, kernel_size=1, padding=1)]
            conf_layers_ += [nn.Conv2d(vgg[v].out_channels * 2,
                                      vgg[v].out_channels, kernel_size=1, padding=0)]
    ...
    for k, v in enumerate(extra_layers):
        if k < indicator or k%2== 0:
            loc_layers_ += [nn.Conv2d(v.out_channels * 2,
                                     v.out_channels, kernel_size=1, padding=0)]
            loc_layers += [nn.Conv2d(v.out_channels, cfg[i]
                                 * 4, kernel_size=3, padding=1)]
            conf_layers_ += [nn.Conv2d(v.out_channels * 2,
                                     v.out_channels, kernel_size=1, padding=0)]
            conf_layers += [nn.Conv2d(v.out_channels, cfg[i]
                                  * num_classes, kernel_size=3, padding=1)]
            i +=1
    return vgg, vgg_flow, extra_layers, (loc_layers, conf_layers, loc_layers_, conf_layers_)
...
if __name__ == '__main__':
    x = torch.autograd.Variable(torch.randn(1, 4, 3, 300, 300)).cuda()
    model = build_net("train").cuda()
    y = model(x)